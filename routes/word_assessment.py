    ...
    try:
        ...
        for page_num in range(len(doc)):
            ...
            prompt = (
                ...
                f"by drawing a single diagonal line from one corner of the box to the opposite corner. "
                f"An empty box or a box with only a small stray mark (not a full diagonal) means incorrect.\n\n"
                f"For each word that appears on this page, return true if the box contains a corner-to-corner "
                f"diagonal line, false if it is empty or has only a stray mark.\n\n"
                f"Return ONLY valid JSON with this structure:\n"
                f'{{"name": "Full Name", "results": {{"word1": true, "word2": false, ...}}}}\n'
                f"true = diagonal line present, false = empty or stray mark only. "
